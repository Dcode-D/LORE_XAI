...
def recognize_features_type(df, class_name):
    ...
    # Add the missing feature to features_type with a default type (e.g., 'string')
    all_features = set(df.columns)
    missing_features = all_features - set(features_type.keys())

    # for col in missing_features:
    #     features_type[col] = 'integer'  # You can adjust the default type as needed
...
def label_decode(df, columns, label_encoder):
    ...
    for col in columns:
        le = label_encoder[col]
        #might remove as type int here
        df_de[col] = le.inverse_transform(df_de[col].astype(int))
    return df_de
